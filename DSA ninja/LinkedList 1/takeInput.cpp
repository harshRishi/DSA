...
// Take input 1
Node *takeInput() // this function will going to return a pointer which is of type Node
{
    // ITS VERY IMPORTANT TO CREATE THIS DYNAMICALLY AS SCOPE OF THIS N IS INSIDE THIS FUNCTION TO TAKE IT OUT WE USED DYNAMIC ALLOCATION
    int data;
    cin >> data;
    Node *head = NULL; // Initailization of head is Null later we'll going to set it as head address
    while (data != -1)
    {
        Node *newNode = new Node(data);
        if (head == NULL)
        { // here we've defined head of the linked list
            head = newNode;
        }
        else
        {
