...
public:
    ...
    Fraction operator++(int) // here nesting is not opossible as it is not allowed so we don't even need to pass it as a refrence
    {
        Fraction fNew(numerator, denominator); // here we have copied the old values of the num and demo
        fNew.simplify();

        numerator = numerator + denominator; // here we've incremented the old values in 'this'  i.e f1 in this case
        this->simplify();
        return fNew;
    }

    Fraction &operator+=(Fraction const &f2) // here if we use void than we can't do nesting i.e (i+=j) += j; so here also we have passed the refrence to 'this' keyword location
    {
        int lcm = this->denominator * f2.denominator;
        int x = lcm / denominator;
        int y = lcm / f2.denominator;
        ...
        denominator = lcm;
        simplify();
        return *this; // here we've returned -this value by using *this
    }
};
